...
# # Sélectionner une province et une maladie
# province_etude = 'KINSHASA'
# maladie_etude = 'PALUDISME'

# # Filtrer les données
# data = df[(df['PROV'] == province_etude) & (df['MALADIE'] == maladie_etude)].copy()
...
#         else:
#             # Décaler la variable climatique
...
#     plt.plot(range(max_lag + 1), corrs, marker='o', label=var)

# plt.xlabel('Décalage (semaines)')
# plt.ylabel('Corrélation avec l\'incidence du paludisme')
# plt.title(f'Corrélations croisées - {maladie_etude} à {province_etude}')
# plt.legend()
...
# print(f"\n=== MEILLEURS DÉCALAGES POUR {province_etude} ===\n")
# for var, corrs in results.items():
#     best_lag = np.argmax(corrs[1:]) + 1  # +1 car on exclut lag 0
#     best_corr = max(corrs[1:])
#     print(f"{var}:")
#     print(f"  Meilleur décalage : {best_lag} semaines")
#     print(f"  Corrélation maximale : {best_corr:.3f}")
#     print(f"  Corrélation à lag 0 : {corrs[0]:.3f}")
#     print()

# # Analyse supplémentaire : matrice de corrélation globale
# print("\n=== MATRICE DE CORRÉLATION (sans décalage) ===\n")
...
# plt.title(f'Matrice de corrélation - {maladie_etude} à {province_etude}')
# plt.show()
...
maladies = ["PALUDISME", "ROUGEOLE", "CHOLERA", "FIEVRE JAUNE"]
...
for maladie in maladies:
    ...
    # --- Agrégation nationale hebdomadaire ---
    data = (
        df[df["MALADIE"] == maladie]
        .groupby("DEBUTSEM")[["INCIDENCE", "PRECTOTCORR", "T2M", "RH2M"]]
        .mean()
        .reset_index()
    ...
    # Création d'une figure unique pour CETTE maladie
    ...
    for var in variables_climat:
        ...
        for lag in range(max_lag + 1):
            ...
            corr = data["INCIDENCE"].corr(data[var].shift(lag))
            correlations.append(corr)

        results[var] = correlations

        # Tracé sur la figure active
        plt.plot(
            range(max_lag + 1), correlations, marker="o", lw=2, label=var
        )

        # Meilleur lag
        best_lag = np.nanargmax(np.abs(correlations[1:])) + 1
        best_corr = correlations[best_lag]

        resultats_globaux.append([maladie, var, best_lag, best_corr])

    # --- Personnalisation du graphique individuel ---
    plt.axhline(y=0, color="black", linewidth=0.8, linestyle="--")

    plt.title(
        f"Corrélations croisées : {maladie} vs Variables Climatiques",
        fontsize=14,
        fontweight="bold",
        pad=15,
    )
    plt.xlabel("Décalage (semaines)", fontsize=11)
    plt.ylabel("Coefficient de Corrélation", fontsize=11)

    plt.xticks(range(max_lag + 1))  # Force l'affichage de tous les lags (0 à 12)
    plt.grid(True, alpha=0.3, linestyle=":")
    plt.legend(fontsize=10, loc="best")

    plt.tight_layout()

    # Sauvegarde d'un fichier propre par maladie (ex: correlations_PALUDISME.png)
    nom_fichier = f"correlations_{maladie.replace(' ', '_')}.png"
